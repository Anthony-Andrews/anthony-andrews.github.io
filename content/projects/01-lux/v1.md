...
While v0 proved that it was at all possible to build custom hardware and firmware, v1 extended that into custom software with more deliberate changes

| Apriltag detection stability   | Apriltag detection accuracy | Apriltag detection speed | Pipeline E2E latency |
| ------------------------------ | --------------------------- | ------------------------ | -------------------- |
| signal to noise                | lens distortion             | mipi csi bandwidth       |                      |
| field of view                  | calibration accuracy        |                          |                      |
| detection algorithm resilience |                             |                          |                      |
| motion blur (& lens focus)     |
| pixels on tag (resolution)     |

---

[Nyquist frequency sampling demo](https://resources.nerdfirst.net/sampling.html)
...
- Minimize lens rated distortion (typically only valuble as a relative measurment) from the manufacturer (if none is given, disregard the lens) - the closer to an ideal "[pinhole](https://en.wikipedia.org/wiki/Pinhole_camera_model)" model lens accheivable, the less error in the final PnP transformation even given calibration.
- Maximize aperature (lower f/x.x) and sensor format size - lets in more light, reduces chromatic aberation and vingetting. Ex. a 1/1.8" format lens will more than cover a 1/2.7" sized sensor, vise-versa leads to vingetting or only a circle being visible.
...
<figure>
<img src="./Screenshot 2026-01-21 at 5.39.35 PM.png" alt="v0" style="width: min(398px, 100%);" />
<figcaption>example of lens distortion accounted for in the mrcal process. a calibrated lens should roughly fit the pinhole model assumption</figcaption>
</figure>
...
<figcaption>f/2.8 4mm M12 lens</figcaption>
</figure>
<figure>
<img src="./csaperture.png" alt="v0" style="width: min(398px, 100%);" />
<figcaption>f/1.3 8.5mm CS lens</figcaption>
</figure>

For the same given exposure, gain, and lighting conditions the difference in brightness and FOV is noticable
